...
POSSIBLE_DOCKER_IMAGES = [
    (ALL_CPU_19_04, 'abeja-inc/all-cpu:19.04'),
    (ALL_CPU_19_10, 'abeja-inc/all-cpu:19.10'),
    (ALL_GPU_19_04, 'abeja-inc/all-gpu:19.04'),
    (ALL_GPU_19_10, 'abeja-inc/all-gpu:19.10'),
    (DockerImageName('fedora', 'httpd', 'version1.0', None), 'fedora/httpd:version1.0'),
    (DockerImageName('fedora', 'httpd', 'version1.0', 'myregistryhost:5000'), 'myregistryhost:5000/fedora/httpd:version1.0'),
    (DockerImageName('fedora', 'httpd', None, None), 'fedora/httpd'),
]
...
def test_with_host() -> None:
    name = DockerImageName('fedora', 'httpd', 'version1.0', 'myregistryhost:5000')
    assert name.repository == 'fedora'
    assert name.name == 'httpd'
    assert name.tag == 'version1.0'
...
def test_cusotm_image() -> None:
    ...
    name = DockerImageName.custom_image(organization_id, repository_name, 'version1.0')

    assert str(name) == 'custom/{}/{}:version1.0'.format(organization_id, repository_name)
