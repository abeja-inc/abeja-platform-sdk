...
class TestSecretManagerAPIClient(unittest.TestCase):
    ...
    def test_get_secrets(self, m):
        ...
        # with parameters
        path_with_params = '/secret-manager/organizations/{}/secrets?offset=10&limit=20&return_secret_value=false'.format(ORGANIZATION_ID)
        m.get(path_with_params, json=SECRETS_RESPONSE)
        ret = client.get_secrets(ORGANIZATION_ID, offset=10, limit=20, return_secret_value=False)
        self.assertDictEqual(ret, SECRETS_RESPONSE)
        ...
        ret = client.get_secrets(ORGANIZATION_ID, return_secret_value=True)
        # 値がデコードされていることを確認
    ...
    def test_get_secret(self, m):
        ...
        ret = client.get_secret(ORGANIZATION_ID, SECRET_ID, return_secret_value=True)
        # 値がデコードされていることを確認
    ...
    def test_create_secret(self, m):
        ...
        expected_payload_with_rotation = {
            'name': SECRET_NAME,
            'value': ENCODED_SECRET_VALUE,
            'rotation': 'true',
            'description': SECRET_DESCRIPTION,
        ...
        self.assertDictEqual(m.request_history[0].json(), expected_payload_with_rotation)
